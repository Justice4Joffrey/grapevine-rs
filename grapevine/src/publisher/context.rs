...
impl<S, P> Drop for PublisherContext<S, P> {
    fn drop(&mut self) {
        self.heartbeat_destructor
            .send(())
            .expect("Failed to send the heartbeat destructor message");
        self.recovery_destructor
            .send(())
            .expect("Failed to send the recovery destructor message")
    }
}
